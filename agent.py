...
def epsilon_nn_greedy(board, possible_moves, possible_boards, player, epsilon):
    ...
    if(possible_boards[bestMove][28] == -15):
        reward = -1.0
        #print(reward)
    print(possible_moves[bestMove])
    return possible_boards[bestMove],possible_moves[bestMove]

def updateNeural(after_state,gamma,epsilon, lam, alpha1, alpha2):
...
def action(board_copy,dice,player,i):
    ...
    after_state,action = epsilon_nn_greedy(board_copy, possible_moves, possible_boards, player, epsilon)
    print(action)
    print(after_state)
    global xold
    ...
    if(actionCount > 1):
        updateNeural(after_state,gamma,epsilon, lam, alpha1, alpha2)
    
    #move = possible_moves[np.random.randint(len(possible_moves))]
    actionCount += 1
    if Backgammon.game_over(after_state):
        global reward
